...
servsoc_ER_all <- csv_files |>
  map_df(
    ~ {
      ...
      # Read file with explicit column types to avoid parsing warnings
      read_delim(.x,
                 delim = ";",
...
# ___ ---------------------------------------------
# 1. LOAD ALL CONFRONTO territori -----------------------------------------

# Directory containing the CSV files
data_dir_cf <- here("data/data_in/ISTAT_SERVSOC/serv_soc_territ")

# List all CSV files
csv_files_cf <- dir_ls(data_dir_cf, glob = "*.csv")

# Read and combine all files
servsoc_cf_all <- csv_files_cf |>
  map_df(
    ~ {
      # Extract territory name from filename
      territorio <- .x |>
        path_file() |>
        path_ext_remove() |>
        str_remove("servsoc_")
      ...
      read_delim(.x,
        ...
        # Add territorio column
        mutate(TERRITORIO = territorio, .before = 1)
    }
  ) |>
  # Drop completely empty columns
  select(where(~ !all(is.na(.))))
...
# Check structure
glimpse(servsoc_cf_all)
dim(servsoc_cf_all)
# Check territories loaded
servsoc_cf_all |> count(TERRITORIO)
# Add labels to main dataset
servsoc_cf_labeled <- servsoc_cf_all |>
  left_join(DATA_TYPE_lookup, by = "DATA_TYPE") |>
  left_join(CATEG_BENEF_lookup, by = "CATEG_OF_BENEFICIARIES") |>
  left_join(CATEG_SERV_lookup, by = "CATEGORY_OF_SERVICE")

# 3. SAVE PROCESSED DATA -----------------------------------------
output_dir_cf <- here("data/data_out/istat_SERVSOC_2011_2022/")
dir_create(output_dir_cf)
# Save full dataset (all services, all data types)
saveRDS(
  servsoc_cf_labeled,
  file.path(output_dir_cf, "servsoc_territ_2011_2022.rds")
)
