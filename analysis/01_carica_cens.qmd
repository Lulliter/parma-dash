...
subtitle: "Materiale intermedio su dati censimento 2023 per comune"
date: last-modified
...
nocite: |
  @istatPopolazioneResidenteClassi2023
---
...
**DATA AGGIORNAMENTO**: Censimento 2023\
**COME OTTENUTO**: Download dati ISTAT CENSIMENTO via API SDMX (vedi `R/f_istat_scarica_cens.R`)\
**CONTENUTO**: + Dataset ID = "IT1,DF_DCSS_POP_DEMCITMIG_TV_2,1.0"\
...
> **NB**: C'e' una IF-THEN_ELSE che non riesegue la chiamata API se trova i dati per Parma gia' salvati in `data/data_out/istat_cens_2023/istat_pop_com_PR_2023.rds`!
...
# 3.b) [SMARTASS VERSION] Solo se istat_pop_com_PR_2023 NON esiste già nella cartella here(data, data_output) , allora scaricalo
file_rds <- here::here("data", "data_out", "istat_cens_2023", "istat_pop_com_PR_2023.rds")

if (file.exists(file_rds)) {
  # Caso 1: il file ESISTE → leggo da disco
  dati_finali_com <- readRDS(file_rds)
  
} else {
  # Caso 2: il file NON esiste → scarico e salvo
  dati_finali_com <- f_scarica_istat_blocchi(
    codici_territorio   = CODICI_TERRITORIO,
    dataset_id          = DATASET_ID,
    anno                = ANNO,
...
# In pratica posso lasciar stare le colonne che non variano: MARITAL_STATUS, AREA_CONTRY_CITIZEN, USUAL_RESID_1Y, NOTE_AREA_CONTRY_CITIZEN, TIME_PERIOD, (NOTE_REF_AREA)  (che però ha NA)
...
# se scaricati di fresco UNCOMMENT già eliminati
...
##### Popolazione per classi decennali di età per comuni di Parma (PR) - Censimento 2023

Quindi filtro per Total o ALL dove non mi interessa la variabile

```{r}
# PEr ogni classe di età dovrei avere il 44 righe
...
CARTELLA_OUTPUT <- here("data", "data_out", "istat_cens_2023")

# Faccio lista di oggetti che iniziano con istat_pop_com_PR_2023 (4 oggetti)
...
> **NB**: C'e' una IF-THEN_ELSE che non riesegue la chiamata API se trova i dati per Parma gia' salvati in `data/data_out/istat_cens_2023/istat_pop_com_ER_2023.rds`!
...
# 3.b) [SMARTASS VERSION] Solo se istat_pop_com_ER_2023 NON esiste già nella cartella here(data, data_output) , allora scaricalo
file_rds_ER <- here::here("data", "data_out", "istat_cens_2023","istat_pop_com_ER_2023.rds")
...
# Solo se ho scaricato tutto di fresco (se no li ho già eliminati)  
...
##### Popolazione per classi di età per comuni di Emilia-Romagna (ER) - Censimento 2023
```{r}
# Per ogni classe di età dovrei avere il 330 righe
istat_pop_com_ER_2023_AGE <-  istat_pop_com_ER_2023 |>
  # AGE CLASS da tenere con valori che cambiano
  dplyr::filter(INDICATOR == "RESPOP_AV" &  
                  GENDER == "T" & 
                  CITIZENSHIP == "TOTAL")  

tabyl(istat_pop_com_ER_2023_AGE$AGE_CLASS)
```

##### Popolazione per sesso per comuni di Emilia-Romagna (ER) - Censimento 2023
```{r}
istat_pop_com_ER_2023_GENDER <-  istat_pop_com_ER_2023 |>
  # GENDER da tenere con valori che cambiano
  dplyr::filter(INDICATOR == "RESPOP_AV" &  
                  AGE_CLASS == "TOTAL" &
                  CITIZENSHIP == "TOTAL")

tabyl(istat_pop_com_ER_2023_GENDER$GENDER)
```
...
istat_pop_com_ER_2023_CITIZENSHIP <-  istat_pop_com_ER_2023 |>
  # CITIZENSHIP da tenere con valori che cambiano
  dplyr::filter(INDICATOR == "RESPOP_AV" &  
                  AGE_CLASS == "TOTAL" &  
                  GENDER == "T")
tabyl(istat_pop_com_ER_2023_CITIZENSHIP$CITIZENSHIP)
```
...
# Faccio lista di oggetti che iniziano con istat_pop_com_ER_2023 (4 oggetti)
...
Obiettivo, aggregare i dati di cui sopra, ma AGGREGATI per provincia (`COD_PROV`) in Emilia-Romagna
...
dati_prov_ER_agg <- dati_prov_ER |>
  dplyr::group_by(
    # CC_UTS, capoluogo o no lo lascio perche è a liv comune 
...
# Rinomino (x coerenza)
istat_pop_prov_ER_2023 <- dati_prov_ER_agg
tabyl(istat_pop_prov_ER_2023$COD_PROV) # 9 PROVINCIE
```                  

##### Popolazione per `classi di età` per PROVINCIA di Emilia-Romagna (ER) - Censimento 2023
```{r}
# Per ogni classe di età dovrei avere il 44 righe
...
```{r}
# Faccio lista di oggetti che iniziano con istat_pop_prov_ER_2023 (4 oggetti)
...
**ISTAT - EsploraDati (IstatData)**\
- **Contenuto**: Dati del Censimento permanente della popolazione 2023\
- **Dataset ID**: `IT1,DF_DCSS_POP_DEMCITMIG_TV_2,1.0`\
- **Variabili principali**: Popolazione residente per comune, disaggregata per sesso, classi di età decennali, cittadinanza, stato civile\
- **Accesso**: Web service SDMX API\
  - Portale: [https://esploradati.istat.it/databrowser/#/it/censpop](https://esploradati.istat.it/databrowser/#/it/censpop)\
  - Query struttura: [Dataflow API](https://esploradati.istat.it/SDMXWS/rest/dataflow/IT1/DF_DCSS_POP_DEMCITMIG_TV_2/1.0/?detail=Full&references=Descendants)\
  - Query URL definite in: `data/istat_queries.yml`\
- **Metodo di acquisizione**: Estrazione via API SDMX tramite funzioni custom `f_scarica_istat()` e `f_scarica_istat_blocchi()` (vedi `R/f_istat_scarica_cens.R`)\
- **Data di riferimento**: Censimento 2023\

---

## Oggetti creati

File salvati in `data/data_out/istat_cens_2023/`:

### Dati comunali - Provincia di Parma (44 comuni)

| File | Descrizione |
|------|-------------|
| `istat_pop_com_PR_2023.rds` | Dati censuari completi per comuni di Parma |
| `istat_pop_com_PR_2023_AGE.rds` | Popolazione per classi di età decennali |
| `istat_pop_com_PR_2023_GENDER.rds` | Popolazione per sesso (M, F, T) |
| `istat_pop_com_PR_2023_CITIZENSHIP.rds` | Popolazione per cittadinanza (italiana, straniera, totale) |

### Dati comunali - Emilia-Romagna (330 comuni)

| File | Descrizione |
|------|-------------|
| `istat_pop_com_ER_2023.rds` | Dati censuari completi per comuni ER |
| `istat_pop_com_ER_2023_AGE.rds` | Popolazione per classi di età decennali |
| `istat_pop_com_ER_2023_GENDER.rds` | Popolazione per sesso (M, F, T) |
| `istat_pop_com_ER_2023_CITIZENSHIP.rds` | Popolazione per cittadinanza |

### Dati provinciali - Emilia-Romagna (9 province)

| File | Descrizione |
|------|-------------|
| `istat_pop_prov_ER_2023.rds` | Dati censuari aggregati per provincia |
| `istat_pop_prov_ER_2023_AGE.rds` | Popolazione per classi di età decennali |
| `istat_pop_prov_ER_2023_GENDER.rds` | Popolazione per sesso (M, F, T) |
| `istat_pop_prov_ER_2023_CITIZENSHIP.rds` | Popolazione per cittadinanza |
