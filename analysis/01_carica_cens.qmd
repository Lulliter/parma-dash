...
```{r}
# 1) Carico shapefile comuni Emilia-Romagna (ER)
...
for (obj_name in list_obj_istat_pop_prov_ER_2023) {
  obj <- get(obj_name)  
  # Definisci i nomi dei file
  rds_file <- file.path(CARTELLA_OUTPUT_prov, paste0(obj_name, ".rds"))
  csv_file <- file.path(CARTELLA_OUTPUT_prov, paste0(obj_name, ".csv"))  
  # Salva come RDS
  saveRDS(obj, file = rds_file)  
  # Salva come CSV
  write.csv(obj, file = csv_file, row.names = FALSE)
